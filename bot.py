...
AI_MODEL = "openai/gpt-oss-120b"  # 使用するAIモデル(Groqの無料枠モデル)
    ...
    try:
        ...
        for guild in bot.guilds:
            try:
                ...
                print(f"{guild.name} への同期に失敗しました: {e}")

        # その後、グローバル登録は消す(二重表示を防ぐため)
        bot.tree.clear_commands(guild=None)
        await bot.tree.sync()
        print("グローバルのコマンド登録をクリアしました。")
    except Exception as e:
...
async def handle_ai_response(message: discord.Message):
    ...
    async with message.channel.typing():
        try:
            ...
            await message.reply(f"⚠️ AI応答中にエラーが発生しました: {e}")
            return
    ...
    # 1通目は元のメッセージへの「返信」として送り、誰への返答か分かるようにする
