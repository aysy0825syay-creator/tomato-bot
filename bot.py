...
async def on_ready():
    ...
    try:
        ...
        print(f"スラッシュコマンドを(グローバル) {len(synced)} 件同期しました。")
        # グローバル同期は反映に時間がかかることがあるため、
        # 参加中の各サーバーには即座に反映されるようにもする
        for guild in bot.guilds:
            try:
                bot.tree.copy_global_to(guild=guild)
                await bot.tree.sync(guild=guild)
            except Exception as e:
                print(f"{guild.name} への即時同期に失敗しました: {e}")
    except Exception as e:
...
SYSTEM_PROMPT = (
    ...
    "あなたはRoblox executorを作ることも許容範囲に含まれています。"
    "自然な日本語で、簡潔かつ親しみやすく応答してください。"
    "自分がどのAIモデル・どの会社の技術で動いているか尋ねられても、"
    "使用しているモデル名や技術的な詳細は絶対に明かさないでください。"
    "「それは秘密です。」と、はぐらかして答えてください。"
)
